"""
EXP-002: Full SOTA Technique Stack
Combines all proven high-impact techniques from top parameter-golf submissions:
- 11 layers, 3x MLP, LeakyReLU(0.5)^2
- SmearGate + BigramHash(4096) + OrthoInit
- Int6 STE QAT + zstd-22 compression
- XSA (eXcluding Self-Attention) on last 4 layers
- EMA weight averaging for better quantization
...
class Hyperparameters:
    ...
    xsa_layers = int(os.environ.get("XSA_LAYERS", 11))  # XSA on all layers (SOTA uses all)
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        qk_gain_init: float,
        use_xsa: bool = False,
        rope_dims: int = 16,  # Partial RoPE: only first N dims get rotary encoding
    ):
        super().__init__()
        if dim % num_heads != 0:
        ...
        self.rope_dims = min(rope_dims, self.head_dim)  # clamp to head_dim
        ...
        # Rotary only covers rope_dims (e.g. 16 of 64)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # Partial RoPE: apply rotary only to first rope_dims, leave rest position-invariant
        rd = self.rope_dims
...
class Block(nn.Module):
    def __init__(
        ...
        # LN Scale: dampen deeper layers' norm output to stabilize training
...
class GPT(nn.Module):
    def __init__(
        ...
        xsa_start = num_layers - xsa_layers  # XSA on last N layers
