"""
EXP-004: Int5 MLP QAT + 12 Layers (Exploratory)
Builds on EXP-003 with int5 MLP quantization:
- Int5 QAT for MLP weights ([-15,15]) — aggressive quantization
- 2x MLP width (not 3x) to fit within 16MB (int5 stored as int8 container)
- 12 layers with CLA2 (same depth as EXP-003)
- Trade-off: narrower MLP but int5 QAT teaches extreme quantization robustness

NOTE: 14L+3xMLP does NOT fit 16MB. Int5 saves compression ratio, not container size.
Size budget: 12L + 2xMLP + CLA2 + int5 = ~15.4MB (verified by calculation)

Target: ~1.08-1.09 BPB (exploratory — may not beat EXP-003 due to 2x MLP vs 3x)
"""
...
class Hyperparameters:
    ...
    num_layers = int(os.environ.get("NUM_LAYERS", 12))  # 12L fits with int5+CLA2 at 2x MLP
    num_kv_heads = int(os.environ.get("NUM_KV_HEADS", 4))
    model_dim = int(os.environ.get("MODEL_DIM", 512))
    num_heads = int(os.environ.get("NUM_HEADS", 8))
    mlp_mult = int(os.environ.get("MLP_MULT", 2))  # 2x MLP to fit 12L+int5 in 16MB
...
class _FakeInt5(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x):
        if x.ndim == 2:
            amax = x.abs().amax(dim=1, keepdim=True).clamp_min(1e-12)
            s = amax / 15.0
            return (torch.clamp(torch.round(x / s), -15, 15) * s).to(x.dtype)
        amax = x.abs().max().clamp_min(1e-12)
        s = amax / 15.0
        return (torch.clamp(torch.round(x / s), -15, 15) * s).to(x.dtype)
...
def fake_int5(x: Tensor) -> Tensor:
    return _FakeInt5.apply(x)
...
def quantize_float_tensor(t: Tensor, use_int6: bool = False, use_int7: bool = False, use_int5: bool = False) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if use_int5:
        qmax = 15   # [-15, 15], 5-bit
    elif use_int6:
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor]):
    ...
    for name, tensor in state_dict.items():
        ...
        q, s = quantize_float_tensor(t, use_int6=is_attn, use_int5=is_mlp)
        if s.ndim > 0:
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if _qat_active:
            w = fake_int5(w)  # MLP uses int5 (aggressive quant → more layers in 16MB)
        x = F.linear(x, w)
